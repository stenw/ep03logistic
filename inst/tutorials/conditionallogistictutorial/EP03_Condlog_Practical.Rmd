...
quiz(
  question("True or false?",
    answer("True", message = 'In unmatched studies you can always user regular logistic regression. Even in matched designs you just need a way to control for the matching factor; this does not always haveb to be contitioning.'),
...
quiz(
  question("True or false",
    answer("True", correct = TRUE),
    answer("False"),
    allow_retry = TRUE, random_answer_order = FALSE, incorrect='The stratifying factors are constant within the strata so cannot beestimated. Interaction effects that include the stratyfying factors and an other covariate generally can.'
  )
)
```
...
quiz(
  question("True or false",
    answer("True"),
    answer("False", correct = TRUE),
    allow_retry = TRUE, random_answer_order = FALSE,  incorrect="Even in normail (unconditional) logistic regression, it is the antilog of the coefficients that can be interpreted as ORs."
  )
)
```
...
# There is also the special argument stata() to indicate the stratum. At the end of the Conditional logistic regression chapter in the course notes an example is given of an analysis done in R.
```
```{r P1-hint-2}
# Use:
endom$OUT<-endom$OUTCOME=='case'
...
Now we also want to look at the dose of oestrogen. However the variable `DOS` missing for some individuals. When we do embedding tests we need to make sure we are comparing the same 
individuals. This can be done using the complete.cases() function.
Add the dose to the model  with EST + GALL + NON + OB + HYP and perform a chi2 test.
