...
quiz(
  question("True or false (check all correct statements)", 
    type='multiple',         
    answer("The confidence interval around an odds ratio is symetric", message="It is only symetric on the log odds scale"),
    answer("The log of the coefficients of an logistic regression can be interpreted as odds ratios", message="Think carefully how to transform the coefficients to odds ratios"),
...
quiz(
  question("What is the probability of responding in the low dose group?",
    ...
    answer("More than 80%", message="To calculate the estimated probabilities simply divide the number of responders in the low dose group to the total number of patients in this group"),
        allow_retry = TRUE, random_answer_order = FALSE
  )
)
```
...
quiz(
  question("What is the standard error of the log odds ratio of the dose?",
    ...
    answer("approximately 1", message = "The se can be computed as the square root of the sum of the inverse of all cell numbers (sqrt(1/a+1/b+1/c+1/d))"),
    answer("approximately 1.5", message = "The se can be computed as the square root of the sum of the inverse of all cell numbers (sqrt(1/a+1/b+1/c+1/d))"),
    answer("approximately 2", message = "The se can be computed as the square root of the sum of the inverse of all cell numbers (sqrt(1/a+1/b+1/c+1/d))"),
    allow_retry = TRUE, random_answer_order = FALSE
  )
)
```
...
The purpose of the excercise is to learn to work with the R functions by which you can perform logistic regression (`glm`, `coef`, `confint`, `anova`, etc.). 
...
Make a cross table of the outcome agvhd against sexmatch using the table function. Add the proportions of healthy and deseased with the prop.table function. Use chisq.test to perform a chi2-test.
