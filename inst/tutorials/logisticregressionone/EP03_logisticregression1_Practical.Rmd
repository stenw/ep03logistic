...
quiz(
  question("True or false (check all that apply)", 
    type='multiple',         
    answer("The logistic regression model can only be used with continuous independent variables"),
    answer("The logistic regression model can only be used with continuous dependent variables"),
    answer("The logistic regression model can only be used with binary independent variables"),
...
```{r Q7, eval=!static}
quiz(
  question("What is the upper limit of the 95% confidence interval of odds ratio of the dose? (high vs low) (use a calculator when needed)",
    ...
    allow_retry = TRUE, random_answer_order = FALSE,
    incorrect="When you know the odds ratio and se of the log odds ratio you can compute a confidence interval. Note that we are interested in the effect of the hogh dose greoup relative to that of the low dose group"
  )
)
```
...
We will now examine how logistic regression can be used to investigate the risk of developing acute graft versus host disease after a bone marrow transplant. We will use the data set BMT.sav. In R you can import the data set using the commands:
```{r, eval=FALSE}
library(foreign)
