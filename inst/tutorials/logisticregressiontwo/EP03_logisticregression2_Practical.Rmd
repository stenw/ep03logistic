...
quiz(
  question("What is the odds ratio of an adverse event of doxycyclin versus roxitromycin?",
    ...
    answer("Between 0.3 and 0.6%"),
    answer("Between 0.6 and 1.5"),
    answer("Between 1.5 and 3", correct=TRUE),
    answer("More than 3", correct=TRUE),
        allow_retry = TRUE, random_answer_order = FALSE
  )
)
```
...
|----------|-----------|---------|-------------|----|--------|-------------|
| AMOXI    | 1.2859    | 0.1447  | 78.97276    | 1  | 0      | 3.61792262  |
| DOXY     | 0.7411    | 0.1538  | 23.21886    | 1  | 0      | 2.09824231  |
| GENDER   | 0.4293    | 0.1068  | 16.15769    | 1  | 0.0001 | 1.53618182  |
...
-2 Log likelihood 2839.873

```{r Q3}
quiz(
  question("Which of the following statements is true? (check all that apply)",
    answer("Age and sex are strong confounders"),
    answer("Gender, age and age squared are jointly significant  (p<0.05)"),
    answer("The probability of having an adverse event is highest for people that are approximately 50 years old (if other variables are constant)", correct=TRUE),
    answer("The upper bound of the odds ratio of gender is approximately 1.9", correct=TRUE),
    allow_retry = TRUE, random_answer_order = FALSE
  )
)
```
...
quiz(
  question("What is the probability of an adverse event of a 50 years old man treated with roxy)",
    ...
    answer("Between 0 and 2 %"),
    answer("Between 2% and 4%", correct=TRUE),
    answer("Between 4% and 10%"),
    answer("More than 10%"),
    allow_retry = TRUE, random_answer_order = FALSE
  )
)
...
Compute the area under the AUC curve for model `glm35` using the prediction and performance functions of the `ROCR` package.
...
```{r P10-hint-1}
# Using the ROCR library you first use the prediction function on the generated predictions
# to make a prediction object. 
# The auc can now be used to calculate the AUC by using the performance function on this object
# with argument, measure='auc.
# NB: the ROCR package is already loaded
```
